...
// Render state is not reset by default because they are many perfectly useful way of altering render state (e.g. changing frameShader/blending settings before an Image call).
...
struct ImDrawList
{
    ...
    // - May be used to alter render state (change sampler, blending, current frameShader). May be used to emit custom rendering commands (difficult to do correctly, but possible).
